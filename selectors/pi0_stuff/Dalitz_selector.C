#define Dalitz_selector_cxx
// The class definition in Dalitz_selector.h has been generated automatically
...
//
// Root > T->Process("Dalitz_selector.C")
// Root > T->Process("Dalitz_selector.C","some options")
// Root > T->Process("Dalitz_selector.C+")
//

#include "Dalitz_selector.h"
#include <TH2.h>
#include <TStyle.h>
...
float tt[12];
//float att[3][8]={0.77,1.5,2.,2.5,3.5,4.25,5.5,8.5,
//                0.49,1.,1.5,2.,2.5,3.5,5.5,8.5,
//                0.35,1.,1.5,2.,2.5,3.5,5.5,8.5};

//cokcok float att[1][5]={1.,2.,3.,4.,5.};
float att[1][5]={0.,1.,2.,3.,4.};

int eslice;

double tmin(double ebeam)
{
float pmass=0.9383;
float jmass=3.511;
...
double ftotmin(double *x, double *p){
return tmin(x[0]);
}

double tmax(double ebeam)
{
float pmass=0.9383;
float jmass=3.511;
float s=pmass*pmass+2.*pmass*ebeam;
...
double xi(double e,double t)
{
if(t<tmin(e)||t>tmax(e))return 0;
double m=0.9383;
double M=3.511;
double s=2.*e*m+m*m;
double p=sqrt((s-(M+m)*(M+m))*(s-(M-m)*(M-m)))/2./sqrt(s);
double q=(s-m*m)/2./sqrt(s);
//cout<<" w, p, q = "<<sqrt(s)<<" "<<p<<" "<<q<<endl;
double ep=sqrt(p*p+m*m);
double eq=sqrt(q*q+m*m);
double pcos = (-t-2.*m*m+2.*eq*ep)/2./q;
double xxi=(eq-ep+q-pcos)/(eq+ep+q+pcos);
return xxi;
}


double Dalitz_selector::ecorr(double pp, double thp, double ebeam)
{
#define p_mass2 0.880354
#define p_mass 0.938272
...
double Dalitz_selector::ftmin(double jmass, double ebeam)
{
float pmass=0.9383;
//float jmass=3.511;
float s=pmass*pmass+2.*pmass*ebeam;
...
double Dalitz_selector::ftmax(double jmass, double ebeam)  {
float pmass=0.9383;
//float jmass=3.511;
float s=pmass*pmass+2.*pmass*ebeam;
float ss=sqrt(s);
...
double Dalitz_selector::itmin(double t)
{
double p[9]={20.9217,-57.542,144.636,-230.799,238.19,-157.904,64.787,-14.948,1.48086};
double fpol=0.;
for (int i=0;i<9;i++){
fpol+=p[i]*pow(t,i);
}
...
double Dalitz_selector::itmax(double t)
{
double p[9]={10.0724,-2.37251,1.14236,-0.28681,0.0470568,-0.00502531,0.000336581,-1.28359e-05,2.12587e-07};
double fpol=0.;
for (int i=0;i<9;i++){
fpol+=p[i]*pow(t,i);
}
...
void Dalitz_selector::Begin(TTree * /*tree*/)
{
...
//hflux = new TFile("flux_20bins.root");
...
fax = (TAxis*)myflux->GetXaxis();
   ...
   char foutname[80];
   sprintf(foutname,"s%dde%d_xi.root",(int)nrsigmas,(int)ndesigmas);
   cout<<foutname<<endl;
...
ntuple = new TNtuple("ntuple","data for event-by-event fit","Cinv:Minv:prompt:t:ebeam:fweight:tweight:tmn:tmx:emn:emx:e1:e2");

}

void Dalitz_selector::SlaveBegin(TTree * /*tree*/)
{
...
events_jp16=951031.00;
events_jp17=946616.00;
events_jp18s=946781.00;
events_jp18f=939316.00;
events_jp=events_jp16+events_jp17+events_jp18s+events_jp18f;
//alex
//events_jp18s=899052.00;


events_bh16=1987198.0;
events_bh17=1955984.0;
events_bh18s=1958584.0;
events_bh18f=1955222.0;
...
//mend=8*0.8/3+mbeg;
   ...
   hweight = new TH2F("hweight","",100,8.2,12.,100,0.,10.3);
   hweight->Sumw2();
   hnoweight = new TH2F("hnoweight","",100,8.2,12.,100,0.,10.3);
   hnoweight->Sumw2();

   hweight_tmn = new TH2F("hweight_tmn","",100,8.2,12.,100,0.,10.3);
   hweight_tmn->Sumw2();
   hnoweight_tmn = new TH2F("hnoweight_tmn","",100,8.2,12.,100,0.,10.3);
   hnoweight_tmn->Sumw2();
...
//
  ...
  ntbins=4;
float ee[2]={8.2,11.5};
  for (int i=0;i<ntbins+1;i++){
    tt[i]=att[eslice][i];
    cout<<" i, tt= "<<i<<" "<<tt[i]<<endl;
  }
  ...
  temin=ee[eslice];
  temax=ee[eslice+1];
  cout<<" temin, temax= "<<temin<<" "<<temax<<endl;
...
// MC
       ...
       sprintf(htit,"MC Minv for %f < t <%f in time (J/#psi)",t1,t2);
...
// data
       ...
       sprintf(htit,"Minv for %f < t <%f in time (J/#psi)",t1,t2);
...
Bool_t Dalitz_selector::Process(Long64_t entry)
{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either Dalitz_selector::GetEntry() or TBranch::GetEntry()
...
//cokcok
//     Minv = Mrec_m;
//cold      t=2.*p_mass*(p_mass-sqrt(pp*pp+p_mass2));
//    pp=pp_m;
//    pep=pep_m;
//    pem=pem_m;
//    thp=thp_m;
...
//cold    bcut_data1=bpove_cut->Eval(Minv)-bpove_cuts->Eval(Minv)*nlsigmas;
    ...
    if(
     ...
     //  &&chi2>0
      //cold &&ebeam>10.&&thep>theta_cut&&them>theta_cut
      //&&ebeam>10.
      &&thep>theta_cut&&them>theta_cut
      //cokcok &&(Cinv-Minv)>0.38&&(Cinv-Minv)<0.44&&abs(Minv-3.08)<0.05
      //&&(Cinv-Minv)>0.38&&(Cinv-Minv)<0.44&&abs(Minv-3.08)<0.05
      //&&(Cinv-Minv)>0.44&&(Cinv-Minv)<0.50&&abs(Minv-3.0923)<0.051
      //&&abs(Minv-3.0923)<0.064
      //notcok &&(Cinv-Minv)>0.37&&(Cinv-Minv)<0.49&&abs(Minv-3.08)<0.05
      //&&abs(Minv-3.08)<0.05
     // cokcok
...
// J/psi case
          www=scale_jpsi;
          //cokcokcok only needed for nnt - MC statistical error estimates:
          //www=1.;
...
// BH case
        ...
        double tmn=abs(ftmin(jmass,ebeam));
        double tmx=abs(ftmax(jmass,ebeam));
        float tmin=tmn;
        float tmax=tmx;
       ...
       if(abs(t)>t1&&abs(t)<t2)break;
        ...
        if(tmx>t2)tmx=t2;
             ...
             //int iebin=floor((ebeam-ebeg)/estep);
             //float ee1=ebeg+iebin*estep;
             //float ee2=ee1+estep;
             float ee1=temin;
             float ee2=temax;
        double emn=ee1;
        ...
        //cokcok
        //www=www/abs(tmx-tmn);
        //woww=www/myflux->GetBinContent(fax->FindBin(ebeam));
        ...
        //cokcokcok if(abs(tmx-tmn)>0.)tweight=1./abs(tmx-tmn);
        //cold if(abs(tmx-tmn)>0.)tweight=1./abs(tmx-tmn);
        if(-(t)<tmin||-(t)>tmax)tweight=1.;

        //cold float fluxint=myflux->GetBinContent(fax->FindBin(ebeam))*(ee2-ee1)/0.045;
        //cold if((!data))fluxint=MCJPfluxALL->GetBinContent(faxMC->FindBin(ebeam))*(ee2-ee1)/0.045;
        float fluxint=myflux->GetBinContent(fax->FindBin(ebeam))/0.100;
        //cold before upd simulations if((!data))fluxint=MCJPfluxALL->GetBinContent(faxMC->FindBin(ebeam))/0.045;
        //fluxint=(ee2-ee1)/0.045;
        //float fluxint=myflux->Integral(fax->FindBin(ee1),fax->FindBin(ee2));
        fweight=1.;
        //cokcokcok if(fluxint>0)fweight=1./fluxint;
        //if(fluxint>0){ fweight=1./fluxint; } else {fweight=0.;};
       
        //cout<<" ee1,ee2,fluxint,fweight= "<<ee1<<" "<<ee2<<" "<<fluxint<<" "<<fweight<<endl;
        ...
        if(-(t)>etmin&&-(t)<etmax){
        float tttt=t;
        ntuple->Fill(Cinv,Minv,prompt,tttt,ebeam,fweight,tweight,tmin,tmax,emin,emax,ee1,ee2); 
        //cout<<" tttt="<<tttt<<" "<<t<<endl;
        }
        //cout<<" t etmin etmax tweight="<<t<<" "<<etmin<<" "<<etmax<<" "<<tweight<<endl;
        ...
        www=www*tweight*fweight;


    if(abs(trf-tbeam)<2.&&(data)&&Minv>3.063&&Minv<3.123)ethist->Fill(ebeam,-t);

        if(Minv>3.03&&Minv<3.15&&(prompt)&&ebeam>8.23){
      cout<<" run number ="<<run_no<<endl;
        hweight->Fill(ebeam,-t,fweight*tweight);
        hnoweight->Fill(ebeam,-t);
        hweight_tmn->Fill(ebeam,-t-tmin,fweight*tweight);
        hnoweight_tmn->Fill(ebeam,-t-tmin);
        }
        if(abs(trf-tbeam)<trf1){
           if(data){
           dminvra->Fill(Mrec_m,www);
           //cok dminva->Fill(Minv,www);
           //cout<<" ebeam,wwwwwwww= "<<ebeam<<" "<<www<<endl;
           dminva->Fill(Cinv,www);
           ...
           for (int ebin=0;ebin<nebins;ebin++){
             ...
             if(ebeam>e1&&ebeam<=e2&&-(t)>etmin&&-(t)<etmax){
               if(data){
           //cout<<" ebein,ebeam,wwwwwwww= "<<ebin<<" "<<ebeam<<" "<<www/deweight<<endl;
               deminva[ebin]->Fill(Cinv,www/deweight);
               deminvap[ebin]->Fill(Minv,www);
               } else {
               eminva[ebin]->Fill(Cinv,www/eweight);
       ...
       float t2=tt[tbin+1];
             ...
             if(-(t+tmin)>t1&&-(t+tmin)<=t2&&ebeam>temin&&ebeam<temax){
             //cokcok if(-(t)>t1&&-(t)<=t2&&ebeam>temin&&ebeam<temax)
             //test if(thp*57.3>t1&&thp*57.3<=t2)
               if(data){
               dtminva[tbin]->Fill(Cinv,www);
               dtminvap[tbin]->Fill(Minv,www);

               //dtminva[tbin]->Fill(Minv,www/dtweight);
               //dtminvap[tbin]->Fill(Minv,www);
               } else {
               tminva[tbin]->Fill(Cinv,www);
           ...
           for (int ebin=0;ebin<nebins;ebin++){
             ...
             if(ebeam>e1&&ebeam<=e2&&-(t)>etmin&&-(t)<etmax){
               if(data){
               deminvb[ebin]->Fill(Cinv,trfd*www/deweight);
               deminvbp[ebin]->Fill(Minv,trfd*www);
               } else {
               eminvb[ebin]->Fill(Cinv,trfd*www/eweight);
...
void Dalitz_selector::SlaveTerminate()
{
...
void Dalitz_selector::Terminate()
{
...
TH1F *dtbh = new TH1F("dtbh","",ntbins,tt);
TH1F *dtjpsi = new TH1F("dtjpsi","",ntbins,tt);
TH1F *dtphi = new TH1F("dtphi","",ntbins,tt);

TH1F *ebh = new TH1F("ebh","",nebins,ebeg,eend);
TH1F *ejpsi = new TH1F("ejpsi","",nebins,ebeg,eend);
TH1F *ephi = new TH1F("ephi","",nebins,ebeg,eend);
TH1F *tbh = new TH1F("tbh","",ntbins,tt);
TH1F *tjpsi = new TH1F("tjpsi","",ntbins,tt);
TH1F *tphi = new TH1F("tphi","",ntbins,tt);
int mcflag;
...
FitPhi(dminva,dminvb,dminvc,dminv,sum,sigma,bg,fitbg);
...
FitPhi(deminva[ebin],deminvb[ebin],deminvc[ebin],deminv[ebin],sum,sigma,bg,fitbg);
...
FitPhi(dtminva[tbin],dtminvb[tbin],dtminvc[tbin],dtminv[tbin],sum,sigma,bg,fitbg);
...
void Dalitz_selector::FitPhi(TH1F*aa, TH1F*bb, TH1F*cc, TH1F*vv, double& ssum, double& ssigma, double& bbg, bool fitbg)
{
double err;
TAxis *xa = (TAxis*)aa->GetXaxis();
float c1=3.53304;
float c2=3.57396;
int bin1=xa->FindBin(c1);
int bin2=xa->FindBin(c2);
float summ=aa->IntegralAndError(bin1,bin2,err,"");
...
void Dalitz_selector::FitJpsi(TH1F*aa, TH1F*bb, TH1F*cc, TH1F*vv,double &ssum, double &ssigma, double &bbg, bool fitbg, int mcflag, double &jpsigma)
{
